...
smap, smap_style, coils, epochs = (
    sys.argv[1],
    sys.argv[2],
    int(sys.argv[3]),
    int(sys.argv[4]),
)
version, blocks, block_depth, filters = (
    sys.argv[5],
    int(sys.argv[6]),
    int(sys.argv[7]),
    int(sys.argv[8]),
...
batch_size, lr, stopper_patience = (
    int(sys.argv[11]),
    float(sys.argv[12]),
    int(sys.argv[13]),
)
...
slice_ids = slice_ids.loc[
    ((slice_ids["slice"] >= 55) & (slice_ids["slice"] <= 201)), :
]  # remove first and last 55 for training
slice_ids_val = slice_ids.loc[
    ((slice_ids["slice"] >= 77) & (slice_ids["slice"] < 178)), :
]  # keep only central 100 for validation + testing
...
if smap_style == "8ch_head":
    masks = [
        "undersampling_masks/218_170/uniform_mask_R=6.npy",
...
for epoch in range(epochs):
    ...
    for i, data in enumerate(train_loader, 0):
        inputs, img_labels, smap_labels = (
            data[0].to(device, dtype=torch.float32),
            data[1].to(device, dtype=torch.float32),
            data[2].to(device, dtype=torch.float32),
        )
        scale_values, input_kspaces, masks = (
        ...
        loss = criterion_mse(output_imgs, img_labels) + criterion_mse(
            output_smaps, smap_labels
        )
    ...
    with torch.no_grad():
        ...
        for i, data in enumerate(valid_loader, 0):
            input, img_label, smap_label = (
                data[0].to(device, dtype=torch.float32),
                data[1].to(device, dtype=torch.float32),
                data[2].to(device, dtype=torch.float32),
            )
            scale_value, input_kspace, mask = (
                data[3].to(dtype=torch.float32),
                data[4].to(device, dtype=torch.complex64),
                data[5].to(device, dtype=torch.float32),
            )
            ...
            loss = criterion_mse(output_img, img_label) + criterion_mse(
                output_smap, smap_label
            )
            val_loss += loss.item()
