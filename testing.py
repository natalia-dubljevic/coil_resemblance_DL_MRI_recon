...
coils, R, smap_style, smap = int(sys.argv[1]), int(sys.argv[2]), sys.argv[3], sys.argv[4]
test_model = int(sys.argv[5])

# model arguments
model_type, version = sys.argv[6], sys.argv[7], 
...
if test_model:
    model = CascadedModel(coils * 2, reps=blocks, block_depth=block_depth, 
                          filters=filters).type(torch.float32)

    checkpoint = torch.load(os.path.join('model_weights', f'{us_style}_{model_type}_model_v{version}.pt'))
...
test_data = SliceTestDataset(slice_ids, 'test', smaps, masks, 'nlinv', coils, 
                             ...
                             target_transforms=target_transforms, snr_factor=snr_factor)
...
for i, data in enumerate(test_loader):
    input_img, img_label, smap_label = data[0].to(device, dtype=torch.float32), data[1].numpy(), data[2].to(device, dtype=torch.float32)
    ...
    if test_model:  # if we want to test the DL model
        ...
        if i in np.arange(0, 10):
            ...
            fig_img.savefig(f'results/model_{us_style}_{model_type}_v{version}_results_accel_{R}_{smap}_snr_{snr}.png')
        ...
        if trad_model == 'CS':
            rec_img_app = L1WaveletRecon(np.squeeze(np.moveaxis(kspace, -1, 0)), np.squeeze(smap_og), max_iter=iters, lamda=lam, show_pbar=False)
        else:
            rec_img_app = SenseRecon(np.squeeze(np.moveaxis(kspace, -1, 0)), np.squeeze(smap_og), max_iter=iters, lamda=lam, show_pbar=False)
...
if test_model:
    ...
    results.to_csv(f'results/model_results_{us_style}_{model_type}_v{version}_accel_{R}_{smap}_snr_{snr}.csv', index=False)

else:
    results = pd.DataFrame({'img_id': img_id, 'R_factor': R, 'smap_style': smap, 
                            'ssim_trad': ssim_trad, 'psnr_trad': psnr_trad, 'phase_trad': phase_trad})
    results.to_csv(f'results/{trad_model}_results_accel_{R}_{smap}_snr_{snr}.csv', index=False)
