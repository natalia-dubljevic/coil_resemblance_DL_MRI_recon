...
smap_layers, smap_filters, lam = int(sys.argv[11]), int(sys.argv[12]), float(sys.argv[13])
...
if test_model:
    ...
    checkpoint = torch.load(os.path.join('model_weights', f'smap_branch_cascaded_model_v{version}.pt'))
...
for i, data in enumerate(test_loader):
    ...
    if test_model:  # if we want to test the DL model
        ...
        if i in np.arange(0, 10):
            ...
            fig_img.savefig(f'results/model_smap_branch_v{version}_results_accel_{R}_{smap}.png')
            ...
            fig_smap.savefig(f'results/model_smap_branch_v{version}_smap_results_accel_{R}_{smap}.png')


    else:  # otherwise, we are doing a CG-SENSE reconstruction
        ...
        if i in np.arange(0, 10):
            ...
            ax.imshow(np.abs(img_label))
            ax.set_title(f'Label')
            plt.savefig(f'results/trad_results_accel_{R}_{smap}.png')
...
if test_model:
    ...
    results.to_csv(f'results/model_results_smap_branch_v{version}_accel_{R}_{smap}.csv', index=False)

else:
    results = pd.DataFrame({'img_id': img_id, 'R_factor': R, 'smap_style': smap, 
                            'ssim_trad': ssim_trad, 'psnr_trad': psnr_trad, 'phase_trad': phase_trad})
    results.to_csv(f'results/trad_results_accel_{R}_{smap}.csv', index=False)
