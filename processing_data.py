...
def slices_to_NLINV(folder_path="/home/pasala/Data/12-channel/"):
    ...
    train_patient_ids = slice_ids.loc[
        slice_ids["split"] == "train", "patient_id"
    ].unique()
    val_patient_ids = slice_ids.loc[slice_ids["split"] == "val", "patient_id"].unique()
    test_patient_ids = slice_ids.loc[
        slice_ids["split"] == "test", "patient_id"
    ].unique()
    ...
    for patient in train_patient_ids:
        for slice in slice_range:
            img_path = (
                folder_path
                + r"train/target_slice_images/"
                + f"{patient}_s{slice}_nlinv_img.npy"
            )
...
if add_to_csv:
    ...
    slice_ids = pd.read_csv(folder_path + "slice_ids.csv")
    slice_ids["nlinv_path"] = (
        folder_path
        + slice_ids["split"].astype(str)
        + r"/target_slice_images/"
        + slice_ids["patient_id"].astype(str)
        + "_s"
        + slice_ids["slice"].astype(str)
        + "_nlinv_img.npy"
    )
    slice_ids = slice_ids.loc[
        (slice_ids["slice"] >= 54) & (slice_ids["slice"] < 201), :
    ]
